...
Read `${CLAUDE_PLUGIN_ROOT}/commands/run_ralph_ri.md` and write it to `${2:-scripts}/RI_LOOP_PROMPT.md`.

This makes the prompt local to the project so it can be customized.

### Step 3: Copy Shell Script

Read `${CLAUDE_PLUGIN_ROOT}/scripts/run_ri_loop.sh` and write it to `${2:-scripts}/run_ri_loop.sh`.
