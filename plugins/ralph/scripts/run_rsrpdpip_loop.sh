#!/bin/bash
# RSRPDPIP Loop - runs RS → RP → Detailed Plan → Implement Plan pipeline
# Source: plugins/ralph/scripts/run_rsrpdpip_loop.sh
#
# Usage: ./run_rsrpdpip_loop.sh <topic> [model]
...
#   3. DP (Detailed Plan) - select highest priority task, create detailed plan
#   4. IP (Implement Plan) - execute the detailed plan
...
while true; do
    ...
    # Phase 3: DP (Detailed Plan)
    echo ">>> Phase 3: DP (Detailed Plan - Auto Select)"
    echo "--------------------------------------------------------------------------------"
    cat "$SCRIPT_DIR/DP_LOOP_PROMPT.md" | claude -p \
        --dangerously-skip-permissions \
        --output-format=stream-json \
        --model "$MODEL" \
        --verbose \
    ...
    # Phase 4: IP (Implement Plan)
    echo ">>> Phase 4: IP (Implement Plan - Auto Find)"
    echo "--------------------------------------------------------------------------------"
    cat "$SCRIPT_DIR/IP_LOOP_PROMPT.md" | claude -p \
        --dangerously-skip-permissions \
        --output-format=stream-json \
        --model "$MODEL" \
        --verbose \
